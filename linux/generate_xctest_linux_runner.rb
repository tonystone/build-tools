...
begin
    options.each do |option, value|
        if option == '--package-path' then package_path=value.dup end
    end
rescue GetoptLong::InvalidOption
    pass
end

package_path << '/' unless package_path.end_with?('/')

#
# Parse the output of switf package dump-package which outputs the Package file in JSON format.
#
package = JSON.parse(`swift package --package-path #{package_path} dump-package`)

# Extract the test targets
test_targets = package["targets"].select { |t| t["isTest"] == true }
...
test_targets.each do |target|
    ...
    target_path = target["path"] || defaultTestsPath(package_path) + module_name

    processTestFiles(module_name, package_path + target_path, 0)
end
...
BEGIN {
    ...
    #
    # Return the test sub path used by the developer
    #
    def defaultTestsPath(package_path)
    ...
    def createLinuxMain(module_name, file_classes, path)
    ...
    def processTestFiles(module_name, path, level)
        ...
        Dir[path + '/*'].each do |sub_directory|
          ...
          if File.directory?(sub_directory)
            sub_classes = processTestFiles(module_name, sub_directory, level + 1)
        ...
        #
        # If its the root level and there are
        # classes, create the LinuxMain file
        #
        if level == 0 && classes.count() > 0
            createLinuxMain module_name, classes, path
        end
...
///
      ...
      string.sub! "<FileName>", File.basename(file_name)
